...
rm -f "${INSTALL_DIR}/lib"/libicui18n.so* 2>/dev/null || true
rm -f "${INSTALL_DIR}/lib/pkgconfig/icu-i18n.pc" 2>/dev/null || true
rm -f "${INSTALL_DIR}/lib/pkgconfig/icu-io.pc" 2>/dev/null || true
# Drop libicuio / libicutest / libicutu — not in registry.
rm -f "${INSTALL_DIR}/lib"/libicuio.so* 2>/dev/null || true
rm -f "${INSTALL_DIR}/lib"/libicutest.so* 2>/dev/null || true
rm -f "${INSTALL_DIR}/lib"/libicutu.so* 2>/dev/null || true
